...
The year is 1995 and focal is 35 years old. Focal and her relatives have all lived in Sweden. 
We would like to know how many female relatives focal has in 1995, their mean age and the number of living kin at each age of focal. Finally, we would like to know at which age she had to suffer a death in average.

## Kin counts in stable populations

We can estimate this in a **stable** framework using data from 1995, assuming that all of focal's relatives experience mortality and fertility from that calendar year (Caswell, 2019). The `DemoKin` package includes data from Sweden as an example (*swe_surv*, *swe_asfr* and *swe_pop*, for survival, fertility and population; type `data(package="DemoKin")`). This data comes from the [Human Mortality Database](https://www.mortality.org/) and [Human Fertility Database](https://www.humanfertility.org/). These datasets were loaded using the`get_HMDHFD` function. 

```{r}
swe35_2015_stable <- kin(U = swe_surv, f = swe_asfr, focal_year = 1995, stable = TRUE)
```
...
We can also visualize the age distribution of relatives when focal is 35:
...
The function also includes data on the mean age of focal's relatives at her actual age:
...
We can visualize the estimated kin counts for focal at age 35 in a stable population using a network diagram:
...
The population of Sweden is, in reality, **non-stable**, and focal' relatives have been expossed to changing mortality and fertility rates.
To account for this, is needed estimattions of kin counts in population with time-variant demographic rates. For this we adapted Matlab´s code from Caswell (2021). Let's take a look at the resulting kin counts at focal's age 35 using empirical single-year mortality and fertility rates as input:
...
Kin loss has studied consequences related to mental health, care support and household incomes over life course. Setting the parameter `alive="no"` we can see how many relatives focal loss during her life (output `kin_death_by_age_focal`), in a stable framework:
...
swe35_2015_nonstable$kin_summary %>%
  filter(alive =="no") %>%
  ggplot() +
  geom_line(aes(age_focal, count_cum, color=cohort))  +
  theme_bw() +
  facet_wrap(~kin,scales="free")
```
