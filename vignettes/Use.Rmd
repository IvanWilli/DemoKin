...
```{r}
swe35_2015_stable <- kins(U = swe_surv, f = swe_asfr, ego_year = 1995, stable = TRUE)
```
...
swe35_2015_stable[["kins_by_age_ego"]] %>% 
  filter(age_ego == 35) %>% 
  select(kin, total, mean_age, sd_age) %>% 
  kable()
```
...
kins_total <- swe35_2015_stable[["kins_by_age_ego"]] %>% 
                filter(age_ego == 35) %>% 
                select(kin, total)
plot_diagram(kins_total)
```
...
```{r, fig.height=6, fig.width=8}
swe35_2015_nonstable <- kins(U = swe_surv, f = swe_asfr, N = swe_pop, stable=FALSE,
                                    ego_cohort = 1960,
                                    selected_kins = c("d","gd","ggd","m","gm","ggm"))

swe35_2015_nonstable$kins %>%
...
swe35_2015_nonstable$kins_alive_death %>%
  filter(alive =="no") %>%
  ggplot() +
