---
title: "TwoExpected kin counts by type of relative: A matrix implementation"
output: 
  html_document:
    toc: true
    toc_depth: 1
...
kin_out %>% 
  ...
  summarise(sex_ratio=sum(count_living[sex=="m"], na.rm=T)/sum(count_living[sex=="f"], na.rm=T)) %>%
...
kin_out_time_variant <- kin2sex(
                      ...
                      sex_focal = "f",time_invariant = FALSE,
                      birth_female = .5,
                      output_cohort = 1900)
